    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="#">后台管理 - 用户维护</a></div>
        </div>
        ...
        <script type="text/javascript">
            $(function () {
			    ...
			    $("#left2RightBtn").click(function(){
			    	...
			    	if ( opts.length == 0 ) {
                        layer.msg("请选择需要分配的角色数据", {time:2000, icon:5, shift:6});
			    	} else {
			    		...
			    		$.ajax({
			    			...
			    			success : function(result) {
			    				if ( result.success ) {
			                        ...
			                        layer.msg("分配角色数据成功", {time:2000, icon:6});
			    				} else {
			                        layer.msg("分配角色数据失败", {time:2000, icon:5, shift:6});
			    				}
			    			}
			    		});
			    	}
			    ...
			    $("#right2LeftBtn").click(function(){
			    	...
			    	if ( opts.length == 0 ) {
                        layer.msg("请选择需要取消分配的角色数据", {time:2000, icon:5, shift:6});
			    	} else {
			    		$.ajax({
			    			type : "POST",
			    			...
			    			success : function(result) {
			    				if ( result.success ) {
			                        ...
			                        layer.msg("取消分配角色数据成功", {time:2000, icon:6});
			    				} else {
			                        layer.msg("取消分配角色数据失败", {time:2000, icon:5, shift:6});
			    				}
			    			}
			    		});
